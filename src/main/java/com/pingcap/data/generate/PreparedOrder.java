...
public class PreparedOrder {
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(PreparedOrder.class);
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            String sql1 = String.format("UPDATE pre_r3_order.%s SET STATUS = 1,modifieddate = NOW() WHERE order_id IN (?,?,?,?,?)", TABLE_NAME);
            String sql2 = String.format("UPDATE pre_r3_order.%s SET STATUS = 2,modifieddate = NOW() WHERE order_id IN (?,?,?,?,?,?)", TABLE_NAME);
            String sql3 = String.format("UPDATE pre_r3_order.%s SET STATUS = 3,modifieddate = NOW() WHERE order_id IN (?,?,?,?)", TABLE_NAME);
            String sql4 = String.format("UPDATE pre_r3_order.%s SET STATUS = 4,modifieddate = NOW() WHERE order_id IN (?,?,?,?)", TABLE_NAME);
            String sql5 = String.format("UPDATE pre_r3_order.%s SET STATUS = 5,modifieddate = NOW() WHERE order_id IN (?,?,?,?)", TABLE_NAME);
            String sql6 = String.format("UPDATE pre_r3_order.%s SET STATUS = 6,modifieddate = NOW() WHERE order_id IN (113034457, 113034458, 113034460, 113031239, 113031241)", TABLE_NAME);
