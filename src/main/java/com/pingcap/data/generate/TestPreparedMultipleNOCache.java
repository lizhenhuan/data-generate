...
public class TestPreparedMultipleNOCache {
    ...
    public static void main(String[] args) throws Exception {
        parseArgs(args);
        List<String> data = new TestPreparedMultipleNOCache().getTestData();
        System.out.println(data.size());

        String driver = "com.mysql.jdbc.Driver";
        String url = String.format("jdbc:mysql://%s/%s?useUnicode=true&characterEncoding=utf-8&useSSL=false&rewriteBatchedStatements=true&useServerPrepStmts=true&cachePrepStmts=true&prepStmtCacheSqlLimit=20480&useConfigs=maxPerformance", IP_PORT, DB_NAME);
    ...
    private static void query(Connection connection, List<String> data, int index) throws Exception {
        ...
        for (int i = start; i < end; i++) {
            ...
            String sql = "select * from t1 where id = '%s'";
            sql = String.format(sql, data.get(i));
            ...
            String prepSQL = "select * from t1 where id = ?";
            PreparedStatement ps =  connection.prepareStatement(prepSQL);
            ps.setString(1, data.get(i));
            ResultSet prepResultSet = ps.executeQuery();
            while (resultSet.next()) {
